...
def add_reviews(df: pd.DataFrame, folder_path: str) -> pd.DataFrame:
    ...
    for filename in REVIEW_FILENAMES:
        ...
        for i, item in enumerate(
            parse_jsonl(os.path.join(folder_path, filename))
        ):
            matches = (df["asin"].values == item["asin"]) & (
                df["reviewerID"].values == item["reviewerID"]
            )
            if any(matches):
                match = df.loc[matches].iloc[0]
                df.loc[matches, "reviewText"] = item["reviewText"]
                df.loc[matches, "sentenceText"] = item["reviewText"][
                    match["start"] : match["end"]
                ]
                counter += 1
            if (i + 1) % 100000 == 0:
                print(f"Parsed {i} items.")
    print(f"Done. \nSuccesfully updated {counter}/{len(df)} items.")
    return df
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--path", "-p", help="Path to the 2018 Amazon dataset folder."
    )
